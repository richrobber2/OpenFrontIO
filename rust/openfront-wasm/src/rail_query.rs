fn run_rail_path(handle: u32, starts: &[TileRef], goal: u32) -> u32 {
...
pub extern "C" fn openfront_map_rail_path(
    ...
    run_rail_path(handle, &starts, goal)
}

/// Allocation-free fast path for the common case of one to four rail starts.
/// Unused scalar lanes are ignored according to `count`.
#[unsafe(no_mangle)]
pub extern "C" fn openfront_map_rail_path_small(
    handle: u32,
    count: u32,
    start0: u32,
    start1: u32,
    ...
    run_rail_path(handle, &starts[..count as usize], goal)
}
