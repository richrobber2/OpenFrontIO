...
export interface OpenFrontWasmExports extends WebAssembly.Exports {
  ...
  openfront_structure_renderer_instance_count(handle: number): number;
  openfront_structure_renderer_data_ptr(handle: number): number;
  openfront_structure_renderer_data_len(handle: number): number;
  ...
  openfront_nuke_trajectory_strip_build(
    p0x: number,
    p0y: number,
    p1x: number,
    p1y: number,
