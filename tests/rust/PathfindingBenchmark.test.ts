...
function bench(label: string, run: () => void): number {
  ...
  console.log(`${label}: ${elapsed.toFixed(3)}ms total (${(elapsed / ITERATIONS).toFixed(4)}ms/query)`);
  return elapsed;
}
...
describe("Rust pathfinding benchmark", () => {
  it("compares warm repeated rail and water queries", async () => {
    ...
    const railMapTs = new GameMapImpl(width, height, railTerrain.slice(), railTerrain.length);
    ...
    for (let y = 4; y < height - 4; y++) {
      if (y % 7 !== 0) waterTerrain[y * width + Math.floor(width / 2)] = land;
    }
    ...
    const waterMapTs = new GameMapImpl(width, height, waterTerrain.slice(), waterTerrain.length);
    ...
    try {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        railMapRust.railPath(Uint32Array.from(railStarts), railGoal);
        waterTs.findPath(waterStarts, waterGoal);
        waterMapRust.waterPath(Uint32Array.from(waterStarts), waterGoal);
      }

      const expectedRail = railTs.findPath(railStarts, railGoal) ?? [];
      const actualRail = Array.from(railMapRust.railPath(Uint32Array.from(railStarts), railGoal));
      expect(actualRail).toEqual(expectedRail);

      const expectedWater = waterTs.findPath(waterStarts, waterGoal) ?? [];
      const actualWater = Array.from(waterMapRust.waterPath(Uint32Array.from(waterStarts), waterGoal));
      ...
      console.log(`Rail Rust/TS speedup: ${(tsRailMs / rustRailMs).toFixed(2)}x`);
      console.log(`Water Rust/TS speedup: ${(tsWaterMs / rustWaterMs).toFixed(2)}x`);
    } finally {
      railMapRust.dispose();
